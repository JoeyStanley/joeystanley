...
<blockquote class="twitter-tweet" data-lang="en"><p lang="en" dir="ltr">In any given project, I think I spend more time trying to figure out how to reshape, melt, and cast my data than anything else.</p>&mdash; Joey Stanley (@joey_stan) <a href="https://x.com/joey_stan/status/799076461013516291">November 17, 2016</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script> 

Here, I make reference to specific functions (`reshape`, `melt` and `cast`) and I'm pretty sure I had spent 6–8 hours working on transforming my data to work in some visualization. I don't even think I ended up getting it done in the end, and I found a really clunky, alternative solution. 
...
You only need to run this once and then it's on your computer. However, even if you already have `tidyverse` installed, it might be a good idea to re-install it because it has had some recent updates. I'll be working with the latest version of the package, which is version 1.2.1 (November 11, 2017). 
...
The colorful messages just let you know what other packages are being loaded at the same time (there should be 8), and what versions. Alternatively, you can read in these packages individually and everything will be fine, but by loading `tidyverse` you can take care of those all in one step.
...
snooze <- read_excel("/Users/joeystanley/Desktop/github/joeystanley/data/snoozing.xlsx")
```
...
```
```{r, echo=FALSE, warning = FALSE}
blank <- read_excel("/Users/joeystanley/Google Drive/R Series/171110-tidyverse/snoozing.xlsx")
```
