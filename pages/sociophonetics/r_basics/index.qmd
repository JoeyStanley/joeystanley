...
* [*An Introduction to R*](https://cran.r-project.org/doc/manuals/R-intro.pdf) by Venables, Smith, and the R Core Team (2017). This is a 99-page PDF that introduces R and some basic skills on how to use it. This version is only a few months old and it a thorough resource for beginners.
...
* Hadley Wickham, the creator of `ggplot2`, has some presentations available online [here](http://ggplot2.org/resources/2007-past-present-future.pdf) and [here](http://ggplot2.org/resources/2007-vanderbilt.pdf).

* The [`ggplot2`](http://ggplot2.tidyverse.org) website is is a good launching pad for other places to find help.

* The book [`ggplot2`: *Elegant Graphics for Data Analysis*](https://www.amazon.com/ggplot2-Elegant-Graphics-Data-Analysis/dp/0387981403) by Wickham is available for purchase, or, if you're a UGA student, available as a free download through the library's website. 

* [Lynda.com](https://www.lynda.com), which is free for UGA students, has some great help for learning R. 
...
You can also use the `seq` function to create more customized sequences. This function, like many others, takes several arguments, some of which are optional. Each argument has a name too, so we can use that name to specify what numbers we're writing belong to which argument.

```{r}
seq(from=1, to=20, by=2)
```

This command creates a sequence of numbers, starting with 1 and going until 20, incrementing number by 2. You can set these numbers to whatever you want and save the whole thing into a variable.
...
In its simplest sense, we can use operators to compre just two numbers. When you run the command, it will return one of two values, `TRUE` or `FALSE`.
