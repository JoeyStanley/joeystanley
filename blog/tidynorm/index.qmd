...
Josef Fruehwald has recently released the [tidynorm](https://jofrhwld.github.io/tidynorm/) package. It "provide[s] convenient and tidy functions to normalize vowel formant data." Since I normalize my data a lot and am entrenched in the [tidyverse](https://www.tidyverse.org), I thought I'd give it a try, especially since I have also written some functions that normalize your data for you and I wanted to compare them. After playing around `tidynorm`, I can confidently say it's a *lot* better than what my versions did. So, the purpose of this post is to 

1. Show a little bit what tidynorm can do.

2. Convince you to switch from the normalization functions in my own packages and to tidynorm.

On this page, I walk through the functions in the tidynorm package that normalized point-based (i.e. not trajectory) formant data. I'll follow along [this article](https://jofrhwld.github.io/tidynorm/articles/norm-methods.html#watt-fabricious-wattevaluationtechniqueimproving2002). As I go through each one, I'll show how you can implement each method using my own packages if applicable, and how it's done in `tidynorm`. Hopefully by the end, I will have convinced you to start adopting `tidynorm`. 
...
I created this dataset for the express purpose of testing out normalization functions.
...
Though Santiago Barreda's work [advises against Lobanov normalization](https://doi.org/10.1017/S0954394521000016), I'll start there because it's most familiar with people. Lobanov normalization is straightforward and can be easily implemented with the `scale` function in base R. Here's a more verbose way of doing things:
...
Here's a more compact way that is functionally equivalent. 
...
Here's how you'd do it in tidynorm:
...
So, it does the exact same thing. The benefit of using `scale` is that it's easier to see what mathematically is happening. The benefit of using `norm_lobanov` is that it's easy to see *why* you're scaling your data and that you're doing the Lobanov normalization method.
...
Note that in this post, I'll use `.silent = TRUE` in my `tidynorm` function calls because not doing so was causing some issues in my R console. But, be sure to look at it in your own code because it provides some good feedback to make sure you've normalized the way you meant to. You can always access that information with `check_norm`. 
...
One (of several) major benefits to `tidynorm` is its consistency in syntax. All three of my normalization procedures had wildly different syntax, which reflected wildly different implementations under the hood. It was not good. `tidynorm`'s functions are all consistent because they actually call a more general `norm_generic`. 
...
Like what we saw with the Lobanov method above, the benefit of using the more raw functions is that you are more closely connected to the math. The downside is that it's prone to error and it's not clear the reason why you do what you do. The benefit of this `norm_barkz` function is that it's all wrapped up into one tidy function and the purpose is clear. In this case the `F1_bz` and `F2_bz` columns correspond to the difference metrics. The `F3_bz` column is not helpful since it's just `Bark(F3) - Bark(F3)`, which is zero, so you can drop it. 
