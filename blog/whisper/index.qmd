...
```{r, eval = FALSE}
av_audio_convert("02-Carol.wav", output = "02-Carol.wav_16bit.wav", format = "wav", sample_rate = 16000)
```
...
```{r, cache = TRUE}
trans <- predict(model, newdata = "02-Carol.wav_16bit.wav", language = "en", n_threads = 12)
```
