...
What you're hearing is center of gravity (CoG). Articulatorily, what's going on is that the area in the mouth that this sound is coming from is between the front of your tongue and the back of your front teeth. In /s/, the tongue and teeth are really close together, creating a really small space. Like we said already, small things create high-pitched sounds. In /ʃ/, the tongue is a little bit further back, which creates a larger area in the mouth, which creates a lower pitch. You may be interested to know that sociolinguists have found that a higher /s/ sound is percieved by listeners to be more feminine.
...
![](images/cog_sit.png){#fig-cog_sit}
...
```
Spectogram > Spectrogram settings…
```

You'll see a window that looks like @fig-cog_spectrogram_settings. 

![](images/cog_spectrogram_settings){#fig-cog_spectrogram_settings}
...
![](images/cog_sit_15K)(#fig-cog_sit_15K)
...
![](images/cog_bash_15K)(#fig-cog_bash_15K)
...
![](images/cog_sit_slice)

With that little sliver highlighted, go to

```
Spectrogram > View Spectral Slice…
```

At first it may look like nothing happened. But, what Praat actually did was create a brand new object. Go back to your **Praat Objects** window and you should see a new "Spectrum" object there. Here's what mine looks like

![](images/cog_spectrum_object.png)

With that new object selected, click on `View & Edit`. You should see something that looks like this:

![](images/cog_spectral_slice_sit.png){@fig-cog_spectral_slice_sit}

As the name suggests, this is a slice of the spectrogram. In this visualization, the *x*-axis goes from 0Hz to 22,050 Hz. Along the *y*-axis now is decibels, a measure of how loud something is. In @cog_spectral_slice_sit, you can see that there's more energy---meaning more volume---in the 4,000--12,000 range, pretty close to what we saw in the regular spectrogram. (Again, there is also a lot of energy in the 0--500Hz range but that was because I wasn't in a sound studio.)

What the CoG is is basically a weighted average measurement for this spectral slice. Frequencies with higher energy will factor into the calculation more prominently and frequences with less energy will influence it less. We can pretty safely guess that the Cog measurement will fall somewhere in that main central zone. 
...
First, something to know is that you can highlight any bit of sound and extract it, either as a new Praat object or as a new file to your computer. To do so, just highlight a portion of the recording you want to save, and go to 

```
Sound > Extract selected sound (time from 0)…
```

If you jump back over to your **Praat Objects** window, you should see a new Sound file. If you play it, it's just the portion that you highlighted. You can then save it and use it for other things if you want. (You can also save a selected portion of sound directly by going to `File > Save selected sound as WAV file…`.)
...
If you are interested in learning more about Praat, there are lots of tutorials out there. 
