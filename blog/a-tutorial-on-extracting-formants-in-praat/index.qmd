...
aliases: 
 - "/blog/a-tutorial-on-extracting-foramnts-in-praat"
categories:
  - How-to Guides
  - Phonetics
  - Praat
...
:::callout-warning
This tutorial is meant to teach you Praat scripting, and uses vowel formant extraction as an example. **I do not recommend that you use the output from this tutorial in serious research** because it blindly extracts acoustic measurements without regard to speaker characteristics and does not validate the output using by-token heuristics. Instead, I strongly recommend using [new-fave](https://forced-alignment-and-vowel-extraction.github.io/new-fave/) for vowel formant extraction.
:::

:::callout-note
I gave a workshop that covered the contents of this workshop. You can find the handout [here](/downloads/191002-formant_extraction.html), which is slightly modified from this blog post.
:::
...
![](praat_basic_script.png){width=30em} 
...
![](praat_hello_world.png){width=30em} 
...
![](praat_info_window1.png){width=30em} 
...
On the outside there is code to indicate where the loop starts and stops. Everything between those lines is what gets executed some number of times. Here, it's going to run 10 times, because we told it to do this <span class="praat"><span class="k">`from`</span>` `<span class="d">`1`</span>` `<span class="k">`to`</span>` `<span class="d">`10`</span></span>. Note that while not necessary, it's a good idea to indent any code that is in the for loop. It just makes the structure of your code easier to see.
...
![](praat_info_window2.png){width=30em} 
